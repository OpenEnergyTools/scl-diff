...
describe("hasher", () => {
    ...
    it("hashes unknown elements by outerHTML", () => {
        ...
        const description = db[nonSCLElement.localName + "@" + nonSCLElement.namespaceURI][digest];
    ...
    it("hashes EnumTypes by EnumVals, Privates and Texts", () => {
        ...
        const description = db.EnumType[digest];
        ...
        expect(priv).to.exist.and.to.have.property("xml", baseEnumType.querySelector("Private")?.outerHTML);
        expect(description).property("@Text").to.have.lengthOf(1);
        const text = db.Text[description["@Text"][0]];
        expect(text).to.exist.and.to.have.property("xml", baseEnumType.querySelector("Text")?.outerHTML);
    });
    after(() => {
        /*
        console.log(
