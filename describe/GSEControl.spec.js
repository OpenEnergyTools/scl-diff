...
describe("Description for SCL schema type tControlWithIEDName", () => {
    it("returns undefined when referenced DataSet is undefined", () => expect(describeGSEControl(invalidDataSet)).to.be.undefined);
    it("returns undefined with missing referenced DataSet", () => expect(describeGSEControl(invalidReference)).to.be.undefined);
    it("returns same description with semantically equal GSEControl's", () => expect(JSON.stringify(describeGSEControl(baseGSEControl))).to.equal(JSON.stringify(describeGSEControl(equalGSEControl))));
    it("returns different description with unequal GSEControl elements", () => expect(JSON.stringify(describeGSEControl(baseGSEControl))).to.not.equal(JSON.stringify(describeGSEControl(diffGSEControl))));
});
