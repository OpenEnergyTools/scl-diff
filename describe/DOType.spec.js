...
describe("Description for SCL schema type DOTypeDescription", () => {
    it("returns undefined with missing cdc", () => expect(DOType(missingCDC)).to.be.undefined);
    it("returns undefined with invalid cdc", () => expect(DOType(invalidCDC)).to.be.undefined);
    it("returns property cdc", () => expect(DOType(baseDOType).cdc).to.equal("MV"));
    ...
    it("returns same description with semantically equal DOType's", () => {
        expect(JSON.stringify(DOType(baseDOType))).to.equal(JSON.stringify(DOType(equalDOType)));
    });
    it("returns different description with unequal DOType elements", () => expect(JSON.stringify(DOType(baseDOType))).to.not.equal(JSON.stringify(DOType(diffDOType))));
});
//# sourceMappingURL=DOType.spec.js.map