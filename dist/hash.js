...
export function hasher(db, eDb, { ignoreAttrs = new Set([
    "desc",
    "id",
    "name",
    "DO.type",
    ...
    function describeReferences(e) {
        const description = {};
        if (!(e.tagName in references))
            return description;
        references[e.tagName].forEach(({ fields, to, scope }) => {
            const candidates = Array.from(e.closest(scope)?.querySelectorAll(to) ?? []);
    ...
    function hash(e) {
        ...
        if (!eDb.h2e.has(digest))
            eDb.h2e.set(digest, new Set().add(e));
...
export function newHasher(options = {}) {
    const db = {};
    const eDb = { e2h: new WeakMap(), h2e: new Map() };
