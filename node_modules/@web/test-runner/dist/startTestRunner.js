...
async function startTestRunner(options = {}) {
    ...
    try {
        const cliArgs = readCliArgsFlag ? (0, readCliArgs_1.readCliArgs)({ argv }) : {};
        const rawConfig = readFileConfigFlag
            ? await (0, readFileConfig_1.readFileConfig)({ configName, configPath: cliArgs.config })
            : {};
        const mergedConfig = (0, mergeConfigs_1.mergeConfigs)(extraConfig, rawConfig);
        const { config, groupConfigs } = await (0, parseConfig_1.parseConfig)(mergedConfig, cliArgs);
