...
const diffDA4 = scl.querySelector(`DOType[id="someDiff"] > DA[desc="someEnumDA"]`);
const equalDA = scl.querySelector(`DOType[id="someDiff"] > DA[desc="someEqual"]`);
describe("Description for SCL schema type DADescription", () => {
    ...
    it("returns same description with semantically equal BDA's", () => expect(JSON.stringify(describeDA(baseDA))).to.equal(JSON.stringify(describeDA(equalDA))));
    it("returns different description with unequal BDA elements", () => expect(JSON.stringify(describeDA(baseDA))).to.not.equal(JSON.stringify(describeDA(diffDA4))));
});
//# sourceMappingURL=DADescription.spec.js.map