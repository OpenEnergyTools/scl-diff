...
let DiffTree = class DiffTree extends LitElement {
    constructor() {
        super(...arguments);
        this.hashers = new WeakMap();
        this.depth = 0;
        this.expanded = false;
    }
    get odd() {
    ...
    get ourDescription() {
        return this.ourHasher?.db[this.ours.tagName][this.ourHash ?? ""];
    }
    get theirDescription() {
        return this.theirHasher?.db[this.theirs.tagName][this.theirHash ?? ""];
    }
    get diff() {
    ...
    renderChildDiffs() {
      ...
      ${Object.entries(this.diff).map(([key, { ours, theirs }]) => {
            ...
            const elementDiff = {};
            ours.forEach((digest) => {
                const element = Array.from(this.ourHasher?.eDb.h2e.get(digest)?.values() ?? []).find((e) => e.tagName === tag);
                if (!element)
                    return;
                const id = identity(element);
                elementDiff[id] ?? (elementDiff[id] = {});
                elementDiff[id].ours = element;
            });
            theirs.forEach((digest) => {
                const element = Array.from(this.theirHasher?.eDb.h2e.get(digest)?.values() ?? []).find((e) => e.tagName === tag);
                if (!element)
                    return;
                const id = identity(element);
                elementDiff[id] ?? (elementDiff[id] = {});
    ...
    renderAttributeDiff() {
        const attrDiff = filterObject(this.diff, ([key]) => !key.startsWith("@") && key !== "eNS");
        const eNSDiff = this.diff.eNS;
        if (!Object.keys(attrDiff).length && !eNSDiff)
            return nothing;
        return html `<table>
      ${Object.entries(attrDiff).map(([name, { ours, theirs }]) => html `<tr tabindex="0">
            <th></th>
            <th>${name}</th>
            <td><span>${ours}</span></td>
            <td><span>${theirs}</span></td>
          </tr>`)}
      ${Object.entries(eNSDiff ?? {}).map(([ns, ks]) => Object.entries(ks).map(([k, { ours, theirs }]) => html `<tr tabindex="0">
              <th>${ns}</th>
              <th>${k}</th>
              <td><span>${ours}</span></td>
              <td>
    ...
    render() {
        ...
        if (!element)
            return nothing;
        let id = (identity(element) || element.tagName).split(">").pop();
        let style = `top: ${this.depth * 24}px; z-index: ${10000 - this.depth};`;
        if (!this.ours)
            style += "color: var(--oscd-primary);";
        if (!this.theirs)
            style += "color: var(--oscd-error);";
...
__decorate([
    property()
], DiffTree.prototype, "ours", void 0);
__decorate([
    property()
], DiffTree.prototype, "theirs", void 0);
__decorate([
    property()
], DiffTree.prototype, "hashers", void 0);
__decorate([
    property({ type: Number })
], DiffTree.prototype, "depth", void 0);
__decorate([
    property({ type: Boolean, reflect: true })
], DiffTree.prototype, "odd", null);
__decorate([
    query("md-icon-button")
], DiffTree.prototype, "expandButton", void 0);
__decorate([
    property({ type: Boolean })
], DiffTree.prototype, "expanded", void 0);
DiffTree = __decorate([
    customElement("diff-tree")
], DiffTree);
export { DiffTree };
//# sourceMappingURL=diff-tree.js.map