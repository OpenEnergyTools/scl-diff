...
function getProgressReport(name, minWidth, finishedFiles, activeFiles, testFiles, passedTests, skippedTests, failedTests) {
    ...
    const progressBar = `${(0, renderProgressBar_1.renderProgressBar)(finishedFiles, activeFiles, testFiles)} ${finishedFiles}/${testFiles} test files`;
...
function getTestProgressReport(config, args) {
    ...
    for (const browser of browsers) {
        ...
        for (const session of sessionsForBrowser) {
            ...
            if (session.status === test_runner_core_1.SESSION_STATUS.FINISHED) {
                ...
                if (testResults) {
                    const parsed = (0, getPassedFailedSkippedCount_1.getPassedFailedSkippedCount)(testResults);
    ...
    if (coverage && coverageConfig) {
        if (testCoverage) {
            ...
            const coverageReport = (0, getCodeCoverage_1.getCodeCoverage)(testCoverage, watch, coverageConfig);
