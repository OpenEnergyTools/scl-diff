...
describe("Description for SCL schema type LDevice", () => {
    it("default nameLength attribute to 32", () => expect(Services(emptyServices)?.nameLength).to.equal(43));
    it("return nameLength attribute", () => expect(Services(equalServices)?.nameLength).to.equal(32));
    ...
    it("return optional ConfReportControl ", () => {
        ...
        expect(Services(diffServices)?.confReportControl?.bufMode).to.equal("buffered");
        ...
        expect(Services(baseServices)?.confReportControl?.bufMode).to.be.equal("both");
    });
    it("return optional ReportSettings ", () => {
        ...
        expect(Services(diffServices)?.reportSettings?.optFields).to.be.equal("Dyn");
        ...
        expect(Services(baseServices)?.reportSettings?.optFields).to.be.equal("Fix");
    ...
    it("returns same description with semantically equal LDevice's", () => expect(JSON.stringify(Services(baseServices))).to.equal(JSON.stringify(Services(equalServices))));
    it("returns different description with unequal LDevice elements", () => expect(JSON.stringify(Services(baseServices))).to.not.equal(JSON.stringify(Services(diffServices))));
});
