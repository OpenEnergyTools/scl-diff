...
describe("Description for SCL schema type LDevice", () => {
    it("returns undefined with missing lnType attribute", () => expect(LDevice(missingLLN0)).to.be.undefined);
    it("returns undefined with missing lnType attribute", () => expect(LDevice(missingLnType)).to.be.undefined);
    it("returns undefined with invalid lnType attribute", () => expect(LDevice(invalidLnType)).to.be.undefined);
    it("returns undefined with missing lnType attribute", () => expect(LDevice(missingInst)).to.be.undefined);
    it("returns undefined with at least one undefined LNDescription", () => expect(LDevice(undefinedLN)).to.be.undefined);
    it("return ldName attribute if present", () => expect(LDevice(diffLDevice)?.ldName).to.equal("ldName"));
    it("return AccessControl child as a string if present", () => expect(LDevice(diffLDevice)?.accessControl).to.equal(`<AccessControl xmlns="http://www.iec.ch/61850/2003/SCL">Some AccessControl Own Namespace thingy!</AccessControl>`));
    it("returns same description with semantically equal LDevice's", () => expect(JSON.stringify(LDevice(baseLDevice))).to.equal(JSON.stringify(LDevice(equalLDevice))));
    it("returns different description with unequal LDevice elements", () => expect(JSON.stringify(LDevice(baseLDevice))).to.not.equal(JSON.stringify(LDevice(diffLDevice))));
});
//# sourceMappingURL=LDevice.spec.js.map