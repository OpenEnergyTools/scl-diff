...
describe("Description for SCL schema element DAType", () => {
    it("returns property iedType with existing iedType attribute", () => {
        expect(DAType(diffDAType1)).to.have.property("iedType", "someIedType");
        expect(DAType(baseDAType)).to.not.have.property("iedType");
    });
    it("returns property bdas recording BDA child elements", () => expect(DAType(baseDAType).bdas.mag).to.satisfy(isAbstractDataAttributeDescription));
    it("returns property protns collecting ProtNs child elements", () => {
        expect(Array.isArray(DAType(baseDAType).protns)).to.be.true;
        expect(DAType(baseDAType).protns[0]).to.satisfy(isProtNsDescription);
    });
    it("returns same description with semantically equal DAType's", () => expect(JSON.stringify(DAType(baseDAType))).to.equal(JSON.stringify(DAType(equalDAType))));
    it("returns different description with unequal DAType elements", () => expect(JSON.stringify(DAType(baseDAType))).to.not.equal(JSON.stringify(DAType(diffDAType1))));
});
//# sourceMappingURL=DAType.spec.js.map