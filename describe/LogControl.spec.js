...
const equalLogControl = scl.querySelector('LogControl[name="equalLogControl"]');
const diffLogControl = scl.querySelector('LogControl[name="diffLogControl"]');
const invalidLogControl = scl.querySelector('LogControl[name="invalidLogControl"]');
const missingLogName = scl.querySelector('LogControl[name="missingLogName"]');
describe("Description for SCL schema type LogControl", () => {
    it("returns undefined with invalid data", () => expect(describeLogControl(invalidLogControl)).to.be.undefined);
    it("returns undefined with invalid logName attribute", () => expect(describeLogControl(missingLogName)).to.be.undefined);
    it("returns same description with semantically equal Control's", () => expect(JSON.stringify(describeLogControl(baseLogControl))).to.equal(JSON.stringify(describeLogControl(equalLogControl))));
    it("returns different description with unequal Control elements", () => expect(JSON.stringify(describeLogControl(baseLogControl))).to.not.equal(JSON.stringify(describeLogControl(diffLogControl))));
});
//# sourceMappingURL=LogControl.spec.js.map