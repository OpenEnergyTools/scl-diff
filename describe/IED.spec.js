...
describe("Description for SCL schema type IED", () => {
    ...
    it("return originalSclVersion attribute defaulting to 2003", () => expect(IED(baseIED)?.originalSclVersion).to.be.equal(2003));
    it("return originalSclRevision attribute defaulting to 'A'", () => expect(IED(baseIED)?.originalSclRevision).to.be.equal("A"));
    it("return originalSclRelease attribute defaulting to 1", () => expect(IED(baseIED)?.originalSclRelease).to.be.equal(1));
    it("return type attribute ", () => expect(IED(diffIED)?.type).to.be.equal("type"));
    it("return manufacturer attribute ", () => expect(IED(diffIED)?.manufacturer).to.be.equal("manufacturer"));
    it("return configVersion attribute ", () => expect(IED(diffIED)?.configVersion).to.be.equal("3"));
    it("return owner attribute ", () => expect(IED(diffIED)?.owner).to.be.equal("owner"));
    it("returns same description with semantically equal IED's", () => expect(JSON.stringify(IED(baseIED))).to.equal(JSON.stringify(IED(equalIED))));
    it("returns different description with unequal IED elements", () => expect(JSON.stringify(IED(baseIED))).to.not.equal(JSON.stringify(IED(diffIED))));
});
