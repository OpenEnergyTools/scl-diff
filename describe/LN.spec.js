...
import { isLNodeTypeDescription } from "./LNodeType.js";
...
const invalidLnTypeDescription = scl.querySelector('*[desc="invalidLnTypeDescription"]');
const baseLLN0 = scl.querySelector(`LDevice[inst="lDevice1"] > *[lnClass="LLN0"][inst=""]`);
const equalLLN0 = scl.querySelector('LDevice[inst="lDevice2"] > *[lnClass="LLN0"][inst=""]');
const diffLLN0 = scl.querySelector('LDevice[inst="lDevice3"] > *[lnClass="LLN0"][inst=""]');
const diffEnumType = scl.querySelector('LDevice[inst="lDevice4"] > *[lnClass="LLN0"][inst=""]');
const baseMMXU = scl.querySelector(`LDevice[inst="lDevice1"] > *[lnClass="MMXU"][inst="1"]`);
const equalMMXU = scl.querySelector(`LDevice[inst="lDevice2"] > *[lnClass="MMXU"][inst="1"]`);
const diffMMXU = scl.querySelector(`LDevice[inst="lDevice3"] > *[lnClass="MMXU"][inst="1"]`);
const diffSDO = scl.querySelector(`LDevice[inst="lDevice4"] > *[lnClass="MMXU"][inst="1"]`);
describe("Description for SCL schema type LN", () => {
    it("returns undefined with missing lnType attribute", () => expect(LN(missingLnType)).to.be.undefined);
    it("returns undefined with invalid lnType attribute", () => expect(LN(invalidLnType)).to.be.undefined);
    it("returns undefined with invalid LNodeType description", () => expect(LN(invalidLnTypeDescription)).to.be.undefined);
    it("return logical node structure in lnType key", () => expect(LN(baseLLN0)?.lnType).to.satisfy(isLNodeTypeDescription));
    it("returns same description with semantically equal LN's", () => {
        expect(JSON.stringify(LN(baseLLN0))).to.equal(JSON.stringify(LN(equalLLN0)));
        expect(JSON.stringify(LN(baseMMXU))).to.equal(JSON.stringify(LN(equalMMXU)));
    });
    it("returns different description with unequal LN elements", () => {
        expect(JSON.stringify(LN(baseLLN0))).to.not.equal(JSON.stringify(LN(diffLLN0)));
        expect(JSON.stringify(LN(baseLLN0))).to.not.equal(JSON.stringify(LN(diffEnumType)));
        expect(JSON.stringify(LN(baseMMXU))).to.not.equal(JSON.stringify(LN(diffMMXU)));
        expect(JSON.stringify(LN(baseMMXU))).to.not.equal(JSON.stringify(LN(diffSDO)));
    });
});
//# sourceMappingURL=LN.spec.js.map