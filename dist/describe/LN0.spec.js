...
const invalidLnTypeDescription = scl.querySelector('LDevice[inst="lDevice7"] > LN0');
...
describe("Description for SCL schema type LN0", () => {
    it("returns undefined with missing lnType attribute", () => expect(LN0(missingLnType)).to.be.undefined);
    it("returns undefined with invalid lnType attribute", () => expect(LN0(invalidLnType)).to.be.undefined);
    it("returns undefined with invalid LNodeType description", () => expect(LN0(invalidLnTypeDescription)).to.be.undefined);
    it("return logical node structure in lnType key", () => expect(LN0(baseLLN0)?.lnType).to.satisfy(isLNodeTypeDescription));
    it("returns same description with semantically equal LN0's", () => {
        expect(JSON.stringify(LN0(baseLLN0))).to.equal(JSON.stringify(LN0(equalLLN0)));
    });
    it("returns different description with unequal LN0 elements", () => {
        expect(JSON.stringify(LN0(baseLLN0))).to.not.equal(JSON.stringify(LN0(diffLLN0)));
        expect(JSON.stringify(LN0(baseLLN0))).to.not.equal(JSON.stringify(LN0(diffEnumType)));
    });
});
//# sourceMappingURL=LN0.spec.js.map