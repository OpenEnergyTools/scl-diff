...
const diffProtNs1 = scl.querySelector('DAType[id="someDifferentDAType1"] > ProtNs');
const diffProtNs2 = scl.querySelector('DAType[id="someDifferentDAType2"] > ProtNs');
const equalProtNs = scl.querySelector('DAType[id="someOtherDAType"] > ProtNs');
describe("Describes the SCL element ProtNs", () => {
    ...
    it("returns equal description with semantically equal ProNs element", () => expect(JSON.stringify(describeProtNs(baseProtNs))).to.equal(JSON.stringify(describeProtNs(equalProtNs))));
    it("returns different description with semantically different ProtNs element", () => {
        expect(JSON.stringify(describeProtNs(baseProtNs))).to.not.equal(JSON.stringify(describeProtNs(diffProtNs1)));
        expect(JSON.stringify(describeProtNs(baseProtNs))).to.not.equal(JSON.stringify(describeProtNs(diffProtNs2)));
    });
});
//# sourceMappingURL=ProtNs.spec.js.map