...
describe("Description for SCL schema type LDevice", () => {
    it("returns undefined with missing lnType attribute", () => expect(Server(invalidServer)).to.be.undefined);
    it("default timeout attribute if present", () => expect(Server(equalServer)?.timeout).to.equal(30));
    it("default timeout attribute if present", () => expect(Server(diffServer)?.timeout).to.equal(13));
    it("returns same description with semantically equal LDevice's", () => expect(JSON.stringify(Server(baseServer))).to.equal(JSON.stringify(Server(equalServer))));
    it("returns different description with unequal LDevice elements", () => expect(JSON.stringify(Server(baseServer))).to.not.equal(JSON.stringify(Server(diffServer))));
});
