...
describe("Description for SCL schema type ReportControl", () => {
    it("returns undefined with invalid data", () => expect(describeReportControl(invalidReport)).to.be.undefined);
    it("excludes invalid ClientLN elements", () => expect(describeReportControl(diffReport)?.clients.length).to.equal(3));
    it("returns same description with semantically equal Control's", () => expect(JSON.stringify(describeReportControl(baseDataSet))).to.equal(JSON.stringify(describeReportControl(equalDataSet))));
    it("returns different description with unequal Control elements", () => expect(JSON.stringify(describeReportControl(baseDataSet))).to.not.equal(JSON.stringify(describeReportControl(diffReport))));
});
//# sourceMappingURL=ReportControl.spec.js.map