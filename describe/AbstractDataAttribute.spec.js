...
const orphanBDA = new DOMParser()
    .parseFromString(`<BDA name="hstVal" desc="refArrayBDA" bType="FLOAT32" count="hstRangeC" />`, "application/xml")
    .querySelector("BDA");
...
const refInvalidSiblingCount = scl.querySelector(`BDA[desc="invalidSiblingCount"`);
...
describe("Description for SCL schema type tAbstractDataAttribute", () => {
    it("returns property sAddr with existing sAddr attribute", () => {
        expect(describeDAorSDAorDAI(baseBDA)).to.have.property("sAddr", "someSAddr");
    ...
    it("returns property bType or 'undefined' if missing ", () => {
        ...
        expect(describeDAorSDAorDAI(missingBType)).to.have.property("bType", "undefined");
    });
    ...
    it("defaults property count 0 if cannot be determined", () => {
        ...
        expect(describeDAorSDAorDAI(refInvalidSiblingCount)).to.have.property("count", 0);
    });
    it("returns property type when bType Enum or Struct", () => {
        ...
        expect(describeDAorSDAorDAI(diffBDA2).type).to.satisfy(isEnumTypeDescription);
    });
    ...
    it("returns same description with semantically equal BDA's", () => expect(JSON.stringify(describeDAorSDAorDAI(baseBDA))).to.equal(JSON.stringify(describeDAorSDAorDAI(equalBDA))));
    it("returns different description with unequal BDA elements", () => expect(JSON.stringify(describeDAorSDAorDAI(baseBDA))).to.not.equal(JSON.stringify(describeDAorSDAorDAI(diffBDA2))));
});
//# sourceMappingURL=AbstractDataAttribute.spec.js.map