...
describe("Description for SCL schema type AccessPoint", () => {
    it("returns undefined with invalid Server", () => expect(AccessPoint(invalidAP1)).to.be.undefined);
    it("return undefined with invalid LN", () => expect(AccessPoint(invalidAP2)).to.be.undefined);
    it("return undefined with invalid ServerAt", () => expect(AccessPoint(invalidAP3)).to.be.undefined);
    it("return router attribute defaulting to false", () => expect(AccessPoint(baseAP)?.router).to.be.false);
    it("return clock attribute defaulting to false", () => expect(AccessPoint(baseAP)?.clock).to.be.false);
    it("return kdc attribute defaulting to false", () => expect(AccessPoint(baseAP)?.kdc).to.be.false);
    it("returns same description with semantically equal LDevice's", () => expect(JSON.stringify(AccessPoint(baseAP))).to.equal(JSON.stringify(AccessPoint(equalAP))));
    it("returns different description with unequal LDevice elements", () => expect(JSON.stringify(AccessPoint(baseAP))).to.not.equal(JSON.stringify(AccessPoint(diffAP))));
});
//# sourceMappingURL=AccessPoint.spec.js.map