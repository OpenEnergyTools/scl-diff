...
describe("Description for SCL schema element Inputs", () => {
    it("returns same description with semantically equal Inputs", () => expect(JSON.stringify(describeInputs(baseInputs))).to.equal(JSON.stringify(describeInputs(equalInputs))));
    it("returns same description with semantically different Inputs", () => expect(JSON.stringify(describeInputs(baseInputs))).to.not.equal(JSON.stringify(describeInputs(diffInputs))));
});
//# sourceMappingURL=Inputs.spec.js.map