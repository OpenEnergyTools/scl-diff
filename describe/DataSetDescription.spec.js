...
describe("Description for SCL schema type DataSet", () => {
    ...
    it("returns same description with semantically equal DataSet's", () => expect(JSON.stringify(describeDataSet(baseDataSet))).to.equal(JSON.stringify(describeDataSet(equalDataSet))));
    it("returns different description with unequal DataSet elements", () => expect(JSON.stringify(describeDataSet(baseDataSet))).to.not.equal(JSON.stringify(describeDataSet(diffDataSet))));
});
//# sourceMappingURL=DataSetDescription.spec.js.map